...
Aplicación multimodal diseñada para procesar, resumir y mejorar metadatos de documentos (PDFs, DOCX, DOC, ODT, ZIPs, RARs, 7Zs, TARs, XMLs y EMLs) usando LLMs avanzados con capacidad de análisis visual.
...
### Flujo de Alto Nivel
1.  **Entrada**: Recibe una referencia a un documento (PDF, DOCX, DOC, ODT, ZIP, RAR, 7Z, TAR, XML o EML) vía API o CLI.
...
2.  **Procesamiento**:
    ...
    *   **ZIP/RAR/7Z/TAR**: Extrae, procesa individualmente los PDFs, DOCX, DOC, ODT, XMLs y EMLs contenidos y **genera un macro-resumen semántico** de toda la colección.
...
graph TD
    ...
    Processor -->|ZIP/RAR/7Z/TAR Macro-Resumen / XML / EML| LLM[LLMService]
    end
...
- **Dependencias opcionales** (incluidas en `requirements.txt`):
...
Procesa todos los archivos PDF, DOCX, DOC, ODT, ZIP, RAR, 7Z, TAR, XML y EML de una carpeta de Google Drive y retorna un JSON con todos los resultados ordenados.
...
# Procesar archivo comprimido específico de una carpeta (ZIP, RAR, 7Z, TAR)
python3 -m app.cli gdrive 1C4X9NnTiwFGz3We2D4j-VpINHgCVjV4Y \
  --file "archivo.zip" \
  --language es \
...
| `LLM_MODEL` | Modelo de texto para macro-resúmenes de archivos comprimidos (ZIP/RAR/7Z/TAR) (vía `LLMService`) | `Qwen/Qwen3-32B` | Sí |
...
### Estrategia de Resumen de PDF y DOCX
...
### Manejo de Archivos Comprimidos (ZIP, RAR, 7Z, TAR)

El servicio soporta múltiples formatos de archivos comprimidos:
- **ZIP**: `.zip` 
- **RAR**: `.rar`, `.cbr`
- **7Z**: `.7z` (requiere `py7zr`)
